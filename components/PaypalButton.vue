<template>
    <div>
        ...
        >
            ...
            <div class="paypal-button-label-container flex">
                <img
                    ...
                    alt
                    class="paypal-logo-card paypal-logo-card- flex-shrink"
                />&nbsp;
                <span class="paypal-button-text true text-white">Debit or Credit Card</span>
            </div>
        </button>
        <transition
        ...
        >
            <PaypalModal @close="showModal = false" v-if="showModal" />
        </transition>
    </div>
</template>
...
export default {
    ...
    head() {
        return {
            script: [{
                src: `https://www.paypal.com/sdk/js?client-id=${this.$config.paypalClientId}&enable-funding=venmo&currency=USD`,
